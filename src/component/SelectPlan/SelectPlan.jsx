		...
		<div className='p-6 flex flex-col gap-8 mobile:w-[90%] mobile:bg-customWhite mobile:mx-auto mobile:rounded-lg'>
			...
			<div className='flex mobile:flex-col gap-3 justify-around'>
				<button
					...
					className={`flex flex-col mobile:flex-row gap-10  pl-4 pr-16 py-4 rounded-md content-start  border ${
						data.plan.name === "Arcade" && "border-marineBlue  bg-magnolia"
					} `}
				>
					<img src='images/icon-arcade.svg' alt='Arcade' className="size-14" />
				...
				<button
					...
					className={`flex flex-col mobile:flex-row gap-10  pl-4 pr-16 py-4 rounded-md content-start  border  ${
						data.plan.name === "Advanced" && "border-marineBlue  bg-magnolia"
					} `}
				>
					<img src='images/icon-advanced.svg' alt='Arcade'  className="size-14" />
					<span className='flex flex-col text-left'>
				...
				<button
					...
					className={`flex flex-col mobile:flex-row gap-10  pl-4 pr-16 py-4 rounded-md content-start  border ${
						data.plan.name === "Pro" && "border-marineBlue  bg-magnolia"
					} `}
				>
					<img src='images/icon-pro.svg' alt='pro'  className="size-14" />
					<span className='flex flex-col text-left'>
			...
			<div className='flex items-center justify-center gap-2  bg-magnolia rounded-md py-3'>
				...
				<span
					className={`${
						data.subscription === "Yearly" ? "text-marineBlue" : "text-coolGray"
					} transition-all font-semibold`}
				>
					Yearly
				</span>
